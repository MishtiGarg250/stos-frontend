...
export default function DocumentsPage() {
  ...
  const { annotations: saved, refetch } = useAnnotations({
    sourceId: activeChatDoc?._id ?? null,
    sourceType: "document",
  });
  
   useEffect(() => { setLocal(saved.map(a => ({ ...a }))); }, [saved]);
  //  const hasUnsaved = local.some(a => a._pending);
  //  const visible    = local.filter(a => a._pending !== "delete");

  //Tool selection
  const selectColor = (color: string) => { setActiveColor(color); setMode("highlight"); };
  const selectErase = () => { setActiveColor(null); setMode("erase"); };
  const deactivate  = () => { setActiveColor(null); setMode("none"); };
  ...
  const handleSave = async () => {
    if (!activeChatDoc || saving) return;
    setSaving(true);
    try {
      ...
      await Promise.all([
        ...
        ...toDelete.map((a) => apiFetch(`/annotations/${a._id}`, { method: "DELETE" })),
      ]);
      await refetch();
    } catch (err) {
      console.error("Save failed:", err);
      alert("Could not save highlights.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleChat = async (e: React.FormEvent) => {
   ...
   const openDoc = (doc: DocumentItem) => {
    if (hasUnsaved && activeChatDoc?._id !== doc._id) {
      if (!window.confirm("You have unsaved highlights. Switch document and discard them?")) return;
    }
   ...
   return (
    <div className="flex flex-col gap-12">
      ...
      <div className="flex justify-between items-center group">
        <div>
          <h2 className="text-7xl text-white font-display font-black tracking-tighter transition-all group-hover:tracking-normal duration-500 uppercase italic">
            ARTIFACTS
          </h2>
          <p className="text-xs font-display text-zinc-500 tracking-[0.4em] uppercase mt-2">
            ENCRYPTED_STORAGE.FS
          </p>
        </div>
        <button onClick={() => setIsDrawerOpen(true)} disabled={isUploading} className="aether-button border-2 border-black">
      ...
      <ZineDrawer isOpen={isDrawerOpen} onClose={() => setIsDrawerOpen(false)} title="UPLINK_PROTOCOL">
        <div className="flex flex-col gap-12">
          <div onClick={() => fileInputRef.current?.click()} className="aether-card border-none bg-zinc-900 p-24 flex flex-col items-center justify-center gap-8 cursor-pointer hover:bg-zinc-800 hover:border-zine-coral border-2 border-dashed border-zinc-700 transition-all group">
            ...
            <div className="text-center">
              ...
              <p className="font-display text-[9px] text-zinc-500 mt-3 uppercase tracking-widest font-bold">PDF, TXT, OR IMAGE // SIZE_LIMIT: 10MB</p>
            </div>
            <input type="file" ref={fileInputRef} onChange={handleUpload} className="hidden" accept=".pdf,.txt,.jpg,.png" />
          </div>
          {isUploading && (
            <div className="aether-card bg-zine-coral/10 border-zine-coral p-8">
              <p className="font-display text-[10px] text-zine-coral font-bold uppercase tracking-widest italic animate-pulse mb-4">Transmitting_Packets...</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className={`aether-card flex flex-col gap-0 p-0 border-zinc-800 ${activeChatDoc ? "lg:col-span-7" : "lg:col-span-12"}`}>
          ...
          <div className="flex flex-col">
            {fetchError ? (
              <div className="p-20 text-center font-display text-sm text-zinc-500 italic uppercase tracking-widest">{fetchError}</div>
            ) : docs.length === 0 ? (
              ...
              <div
                key={doc._id}
                onClick={() => openDoc(doc)}
                className={`grid grid-cols-12 items-center p-8 border-b border-zinc-900 hover:bg-zinc-900/50 transition-all cursor-pointer group ${
                  activeChatDoc?._id === doc._id ? "bg-zinc-900 border-l-8 border-zine-coral" : "border-l-8 border-transparent"
                }`}
              >
        ...
        {activeChatDoc && (
          <div className="lg:col-span-5 aether-card border-zine-coral bg-zine-paper text-zinc-950 flex flex-col min-h-[600px] overflow-hidden animate-in fade-in zoom-in-95 duration-500">
            <div className="p-6 bg-zinc-900 text-white flex justify-between items-center border-b-4 border-zinc-100 flex-shrink-0">
              <span className="font-display text-[10px] text-zinc-400 font-black uppercase tracking-[0.3em] truncate max-w-[200px]">
                Node Analysis: {activeChatDoc.name}
              </span>
              <button
                onClick={() => { setActiveChatDoc(null); setChatAnswer(""); setMode("none"); setActiveColor(null); }}
                className="text-zinc-500 hover:text-zine-coral transition-colors font-black"
              >
                X
              </button>
            </div>

            {/* Annotation toolbar — shown once there is AI content to annotate */}
            {chatAnswer && (
              <>
                <AnnotationToolbar
                  mode={mode}
                ...
                />
                {mode !== "none" && (
                  <div className={`flex-shrink-0 px-4 py-1 text-[7.5px] font-mono font-bold uppercase tracking-[0.25em] border-b ${
                    mode === "highlight" ? "bg-amber-950/20 border-amber-900/30 text-amber-400/80"
                    : "bg-red-950/20 border-red-900/30 text-red-400/80"
                  }`}>
                    {mode === "highlight"
                      ? `● ${HIGHLIGHT_COLORS.find(c => c.value === activeColor)?.name ?? ""} pen — select text`
                      : "● Eraser — click a highlight to remove"}
                  </div>
                )}
              </>
            )}

            {/* AI answer — annotatable */}
            <div className="flex-1 p-10 overflow-y-auto max-h-[400px] relative">
              {chatAnswer ? (
                <div className="flex flex-col gap-6">
                  <p className="font-display text-[9px] font-black text-zine-coral uppercase tracking-widest italic">
                    AI_Response_Buffer:
                  </p>
                  <AnnotatedContent
                    content={chatAnswer}
                    ...
                    onErase={handleErase}
                    className="font-sans text-lg leading-relaxed text-zinc-800 font-bold opacity-90 italic"
                  />
                </div>
              ) : (
            ...
            <form onSubmit={handleChat} className="p-8 border-t-4 border-zinc-900 bg-zinc-100 flex-shrink-0">
              <textarea
                value={chatQuery}
              ...
              />
              <button type="submit" disabled={isChatting} className="aether-button w-full h-16 text-xl tracking-[0.2em] shadow-none hover:bg-zinc-900 hover:text-white">
